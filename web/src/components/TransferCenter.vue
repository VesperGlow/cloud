...
const clearableCount=computed(()=>props.uploads.filter(task=>task.status==='done'||task.status==='cancelled').length+props.downloads.filter(task=>task.status!=='starting').length)
  ...
  <details ref="center" class="transfer-center">
    <summary :title="activeCount?`${activeCount} 个传输任务进行中`:'上传与下载'" aria-label="打开传输中心">
      <svg class="transfer-ring" viewBox="0 0 40 40" aria-hidden="true">
        ...
        <g class="ring-transfer">
          <path d="M15 12v16m0 0-3.5-3.5M15 28l3.5-3.5"/>
          <path d="M25 28V12m0 0-3.5 3.5M25 12l3.5 3.5"/>
        </g>
      </svg>
      <span v-if="activeCount" class="transfer-count">{{ activeCount }}</span>
    </summary>
    <section class="transfer-popover">
      <header><div><strong>传输中心</strong><small>{{ activeCount?`${activeCount} 项进行中`:'最近任务' }}</small></div><button v-if="clearableCount" @click.prevent="$emit('clear')">清除已完成</button></header>
      <div v-if="!allTasks" class="transfer-empty"><svg viewBox="0 0 40 40" aria-hidden="true"><path d="M15 12v16m0 0-3.5-3.5M15 28l3.5-3.5"/><path d="M25 28V12m0 0-3.5 3.5M25 12l3.5 3.5"/></svg><p>还没有上传或下载任务</p></div>
...
.transfer-empty{display:grid;place-items:center;padding:34px;color:#94a3b8}.transfer-empty svg{width:40px;height:40px;fill:none;stroke:#8fa6c2;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}.transfer-empty p{margin:8px 0 0;font-size:12px}@keyframes spin{to{transform:rotate(270deg)}}
