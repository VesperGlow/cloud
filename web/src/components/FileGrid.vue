...
function openItem(item:DriveFile){if(canOpen(item))emit('open',item)}
function activate(item:DriveFile,event:MouseEvent){
  if(event.detail>1)return
  if(canOpen(item)&&window.matchMedia('(max-width: 850px), (hover: none), (pointer: coarse)').matches)openItem(item)
...
<template>
  <div class="file-grid">
    <article v-for="item in items" :key="item.id" class="file-card" :class="{mutedrow:item.status!=='ready',selected:selectedIds.has(item.id)}" role="button" tabindex="0" :aria-label="`${item.name}，${selectedIds.has(item.id)?'已选择':'未选择'}`" @click="activate(item,$event)" @dblclick="openItem(item)" @keydown.enter.prevent="openItem(item)" @keydown.space.prevent="$emit('select',item)">
