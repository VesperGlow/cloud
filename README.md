...
- 缩略图：图片与 EPUB 封面由服务端按需重采样、视频由内置 ffmpeg 抽帧，全部作为内容寻址派生对象持久化在 S3 的 `thumbs/` 前缀，前端用内容哈希版本化的不可变 URL 长期缓存；视频抽帧一次会完整下载该视频（每内容仅一次，上限 2 GiB），无 ffmpeg 时前端回退为浏览器抽帧上传。
...
- 单块媒体（4 MiB 以下）直读 S3；多块媒体经服务端流式拼接，无服务端缩略图、转码、波形、OCR 或 EXIF 索引，不受浏览器支持的编码无法播放。超大文件（数 TiB）的清单解析与逐块校验开销随块数线性增长，建议为超大媒体库调大 `BLOCK_SIZE`。
