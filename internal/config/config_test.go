...
func TestLoadDefaults(t *testing.T) {
	...
	if c.S3Region != "us-east-1" || c.S3PathStyle || c.PresignExpires != 15*time.Minute || c.UploadExpires != 24*time.Hour || c.GCInterval != time.Hour || c.BlockSize != 4<<20 {
		t.Fatalf("defaults: %+v", c)
	}
...
func TestLoadOverrides(t *testing.T) {
	...
	if !c.CookieSecure || !c.S3PathStyle || c.BlockSize != 8<<20 || c.FFmpegPath != "/usr/bin/ffmpeg" {
